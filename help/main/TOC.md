---
user-guide-title: "[!DNL Journey Optimizer B2B Edition] Tutorials"
breadcrumb-title: "[!DNL Journey Optimizer B2B Edition] Tutorials"
user-guide-description: Learn how to get the most out of [!DNL Journey Optimizer B2B Edition]. Orchestrate account and buying group journeys using built-in generative AI and industry-leading automation to maximize demand for specific offerings.
auto-video-transcripts: true
---

# [!DNL Journey Optimizer B2B Edition] {#tutorials}

+ [[!DNL Journey Optimizer B2B Edition] Tutorials](overview.md)
+ [[!DNL Journey Optimizer B2B Edition] overview](overview-video.md)
+ Account journeys {#account-journeys}
  + [Introducing account journeys](/help/main/account-journeys/introducing-account-journeys.md)
  + [Create an account journey](/help/main/account-journeys/create-an-account-journey.md)
  + Journey nodes {#journey-nodes}
    + [Action node](/help/main/account-journeys/journey-nodes/action-node.md)
    + [Listen node](/help/main/account-journeys/journey-nodes/listen-node.md)
    + [Listen for AEP events](/help/main/account-journeys/journey-nodes/listen-for-aep-events.md)
    + [Paid media orchestration](/help/main/account-journeys/journey-nodes/paid-media-orchestration.md)
    + [Split account journey](/help/main/account-journeys/journey-nodes/split-account-journey.md)
+ Accounts {#accounts}
  + [Account lists](/help/main/accounts/account-lists.md)
+ Buying groups {#buying-groups}
  + [Buying groups overview](/help/main/buying-groups/buying-groups-overview.md)
  + [Buying group stages](/help/main/buying-groups/buying-group-stages.md)
  + [Create a buying group](/help/main/buying-groups/create-a-buying-group.md)
  + [Role templates](/help/main/buying-groups/role-templates.md)
  + [Solution interest](/help/main/buying-groups/solution-interest.md)
  + [Using the Account Qualification Agent](/help/main/buying-groups/account-qualification-agent.md)
